      ...
      <div className="mx-4 md:mx-48">
        ...
        <div className="flex space-x-6 overflow-x-scroll scrollbar-hide lg:grid lg:grid-cols-3 lg:gap-2 lg:overflow-hidden pb-4 mt-4">
          {[1, 2, 3].map((guide, index) => {
            const img_key_name = `shop_image_${guide}`;
            const caption_key_name = `shop_image_${guide}_caption`;
            return (
              <div
                key={index}
        ...
        {!_.isEmpty(activity?.shop_pro_tip_image) &&
          !_.isEmpty(activity?.shop_pro_tip) && (
            <ProTip
              city_name={activity?.city_name}
              title={activity?.shop_pro_tip}
              image={activity?.shop_pro_tip_image?.filename_disk}
            />
          )}
      </div>
    </section>
...
export default ShopActivitySection;
